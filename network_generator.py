...
def create_pattern(num_leaves, p, tree_type='regular'):
    pattern_sz= clog2(num_leaves)
    pattern= ''
    ...
    if(tree_type == 'regular'):
        ...
        assert(p==0.5) # only valid when p==0.5
        for i in range(0, pattern_sz):
            if(i%4 == 0 or i%4 == 1):
                pattern += '2'
            else:
                pattern += '1'
...
def create_leafless_network(num_leaves, p, num_sent_per_leaf, args):
    pattern= create_pattern(num_leaves, p, args.tree_type)
...
# Didn't check this function with new create_pattern with tree_type input variable
